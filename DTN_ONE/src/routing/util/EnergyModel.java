...
public class EnergyModel implements ModuleCommunicationListener {
	...
	public EnergyModel(Settings s) {
		...
		this.baseEnergy = s.getDouble(BASE_ENERGY_S);	
	...
	public void reduceDiscoveryEnergy() {
		reduceEnergy(this.scanResponseEnergy+this.baseEnergy);
	}
	...
	public void update(NetworkInterface iface, ModuleCommunicationBus comBus) {
		...
		if (simTime > this.lastUpdate && iface.isTransferring()) {
			...
			reduceEnergy(delta * (this.transmitEnergy+this.baseEnergy));
		}
		...
		if (iface.isScanning()) {
			...
			if (iface.getTransmitRange() > 0) {
				if (delta < 1) {
					reduceEnergy((this.baseEnergy+this.scanEnergy) * delta);
				} else {
					reduceEnergy(this.baseEnergy+this.scanEnergy);
				}
			}
		}
		else if (!iface.isActive()){
			if (delta < 1) {
				reduceEnergy(this.baseEnergy * delta);
			} else {
				reduceEnergy(this.baseEnergy);
			}
		}
	}
